...
class LSTM(nn.Module):
    ...
    def __init__(self, seq_length, input_dim, hidden_dim, num_classes,
        ...
        #weight matrix
        self.W_gx = nn.Parameter(torch.zeros(hidden_dim, input_dim))
        nn.init.kaiming_normal_(self.W_gx)
        self.W_gh = nn.Parameter(torch.zeros(hidden_dim, hidden_dim))
        nn.init.kaiming_normal_(self.W_gh)
        self.W_ix = nn.Parameter(torch.zeros(hidden_dim, input_dim))
        nn.init.kaiming_normal_(self.W_ix)
        self.W_ih = nn.Parameter(torch.zeros(hidden_dim, hidden_dim))
        nn.init.kaiming_normal_(self.W_ih)
        self.W_fx = nn.Parameter(torch.zeros(hidden_dim, input_dim))
        nn.init.kaiming_normal_(self.W_fx)
        self.W_fh = nn.Parameter(torch.zeros(hidden_dim, hidden_dim))
        nn.init.kaiming_normal_(self.W_fh)
        self.W_ox = nn.Parameter(torch.zeros(hidden_dim, input_dim))
        nn.init.kaiming_normal_(self.W_ox)
        self.W_oh = nn.Parameter(torch.zeros(hidden_dim, hidden_dim))
